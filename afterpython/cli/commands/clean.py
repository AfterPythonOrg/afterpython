...
@click.pass_context
@click.option('--all', is_flag=True, help='--all passed to "myst clean", also clean afterpython build directories (_build/ and _website/build/)')
def clean(ctx, all: bool):
    ...
    if all:
        ...
        click.echo("Done! Use 'ap clean --all' to also remove afterpython build directories.")